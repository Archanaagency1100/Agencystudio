(function () {
    ...
    function recolour(root) {
        const elements = root.nodeType === 1 ? [root].concat(Array.from(root.querySelectorAll("*"))) : Array.from(document.querySelectorAll("*"));
    ...
    function schedule(root) {
        ...
        framePending = true;
        requestAnimationFrame(function () { framePending = false; recolour(root || document); });
    }
    ...
    document.addEventListener("pointerover", function (event) { schedule(event.target); }, true);
    document.addEventListener("focusin", function (event) { schedule(event.target); }, true);

    new MutationObserver(function (mutations) {
        const added = mutations.find(function (mutation) { return mutation.addedNodes.length; });
        schedule(added && added.target.nodeType === 1 ? added.target : document);
